...
const app = express();
const PORT = process.env.PORT || 3000; // Ready for Render hosting
...
try { db.exec(`ALTER TABLE users ADD COLUMN last_seen TEXT`); } catch(e){} // Using TEXT to store precise JS Date.now()
...
// ONLINE TRACKER: Automatically update 'last_seen' using precise Node.js time
app.use((req, res, next) => {
  if (req.session && req.session.user) {
    try { 
      db.prepare("UPDATE users SET last_seen = ? WHERE id = ?").run(Date.now().toString(), req.session.user.id); 
    } catch(e) {}
  }
  next();
});
...
app.post('/api/auth/google-login', async (req, res) => {
  ...
  try {
    ...
    // Check Role Logic (Sub-admins can login even if can_manage_admins is 0, as long as role is 'admin')
...
app.get('/api/admins', requireAdmin, (req, res) => {
  ...
  const admins = db.prepare("SELECT * FROM users WHERE role='admin' ORDER BY id ASC").all();
  const currentMs = Date.now();
  
  // Calculate online status accurately without SQLite Timezone issues
  const result = admins.map(a => {
    let seconds_since_active = 9999;
    if (a.last_seen) {
      seconds_since_active = (currentMs - parseInt(a.last_seen)) / 1000;
    }
    return {
      id: a.id, email: a.email, name: a.name, is_super_admin: a.is_super_admin,
      can_add_food: a.can_add_food, can_serve_orders: a.can_serve_orders,
      can_manage_admins: a.can_manage_admins, seconds_since_active: seconds_since_active
    };
  });
...
app.post('/api/admins', requireAdmin, (req, res) => {
  if (!req.session.user.can_manage_admins) return res.status(403).json({ error: 'Forbidden' });
  const email = req.body.email.toLowerCase(); // Force lowercase to avoid mismatch bugs
  const { can_add_food, can_serve_orders, can_manage_admins } = req.body;
...
app.delete('/api/admins/:id', requireAdmin, (req, res) => {
  ...
  try {
    ...
    // 1. Unlink their orders so we don't break the history page
    db.prepare("UPDATE orders SET user_id = 0 WHERE user_id = ?").run(targetId);
    // 2. Force delete the user from the database completely
    db.prepare("DELETE FROM users WHERE id=? AND is_super_admin=0").run(targetId);
    res.json({ success: true });
  } catch (error) {
    console.error(error);
